...
import { ArrowRight, CheckCircle, CreditCard, DollarSign, MapPin, ShoppingCart } from 'lucide-react'
...
const Checkout = () => {
  ...
  if(orderPlaced) {
    return (
        <div className='min-h-screen bg-linear-to-br pt-28 from-[#43C6AC] to-[#F8FFAE] py-20'>
            <div className='container mx-auto px-4 max-w-4xl'>
                <div className='bg-white rounded-2xl shadow-xl p-8 md:p-12 text-center'>
                    <div className='inline-flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-6'>
                        <CheckCircle className='w-12 h-12 text-green-500' />
                    </div>
                    <h1 className='text-3xl md:text-4xl font-bold bg-linear-to-r from-[#1A237E] to-[#43C6AC] bg-clip-text text-transparent mb-4'>
                        Order Confirmed!
                    </h1>
                    ...
                    <p className='text-gray-600 mb-8'>
                        We've sent a confimation email to <span className='font-medium'>{formData.email}</span>.
                    ...
                    <div className='flex flex-col sm:flex-row gap-4 justify-center'>
                      <Link to='/books' className='px-6 py-3 bg-linear-to-r from-[#1A237E] to-[#43C6AC] text-white rounded-lg
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className='mb-6'>
                                ...
                                {formFields.map((row, rowIndex) => (
                                  <div key={rowIndex} className={`grid grid-cols-1 ${row.some((field) => field.fullWidth)
                                    ? ""
                                    : "md:grid-cols-2"
                                    ...
                                    {row.map(field => (
                                      <div key={field.name} className={field.fullWidth ? 'col-span-full' : ''}>
                                        ...
                                        className='w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#43C6AC]
                                        focus:border-transparent' required={field.required} />
                                      </div>
                                    ))}
                                  </div>
                                ))}
