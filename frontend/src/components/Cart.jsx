...
import { ArrowBigRight, BookOpen, Minus, Plus, ShoppingBag, Trash2 } from 'lucide-react'
...
const Cart = () => {
    ...
    const inc = (item) => updateCartItem({id: item.id, source: item.source, quantity: item.quantity + 1 })
    const dec = (item) => {
        const newQty = item.quantity - 1;
        if (newQty > 0) {
            updateCartItem({id: item.id, source: item.source, quantity: newQty })
        }
        else{
    ...
    <div className={styles.container}>
        <div className={styles.wrapper}>
                        ...
                        {cart.items.map((item) => (
                            <div key={`${item.source}-${item.id}`} className={styles.cartItemCard}>
                                <div className={styles.cartItemContent}>
                                    <img src={getImageSrc(item)} alt={item.title} className={styles.cartItemImage} />
            ...
            <Link to='/checkout' className={styles.checkoutBtn}>
                Checkout Now
                <ArrowBigRight className={styles.checkoutIcon} />
            </Link>
