    ...
    useEffect(() => {
        const fetchBooks = async () => {
            ...
            try {
                ...
                setError(err.meassage || 'Failed to load books')
            }
            finally {
                setLoading(false)
            }
                        ...
                        return (
                            <div key={book._id} className={styles.bookCard}>
                                <div className={styles.imageWrapper}>
                                    <img src={book.image.startsWith('http') ? book.image : `${API_BASE}${book.image}`} alt={book.title} className={styles.image} />

                                    <div className={styles.rating}>
                                        {[...Array(5)].map((_, i) => (
                                            <Star className={`h-4 w-4 ${i < Math.floor(book.rating || 0) ? 'text-[#43C6AC] fill-[#43C6AC]'
                                                : 'text-gray-300' }`} key={i} />
                                        ))}
                                    </div>
                                </div>
                                ...
                                <span className={styles.actualPrice}>₹{book.price.toFixed(2)}</span>
                                    ...
                                    <div className={styles.qtyBox}>
                                        ...
                                        <buttton onClick={() => handleAdd(book)} className={styles.addBtn}>
